...
class ConfigGroup(models.Model):
    ...
    def set(self, key, val, description=''):
        for ci in self.configitem_set.all():
            if ci.key.lower() == key:
                ci.value = val
                ci.save()
                return
        ConfigItem(group=self, key=key, value=val, desc=description).save()
...
class ConfigItem(models.Model):
    ...
    desc = models.CharField('Description', max_length=100)
    typ = models.PositiveIntegerField('Type of value', choices=TYP_CHOICES, default=1)
